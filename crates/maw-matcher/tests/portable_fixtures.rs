...
#[test]
fn fixtures_feature_exports_the_same_corpora() {
    assert_eq!(
        maw_matcher::MATCHER_RESOLVE_TARGET_FIXTURES_JSON,
        include_str!("fixtures/matcher-resolve-target.fixtures.json")
    );
    assert_eq!(
        maw_matcher::NORMALIZE_TARGET_FIXTURES_JSON,
        include_str!("fixtures/normalize-target.fixtures.json")
    );
}
